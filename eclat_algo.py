...
def eclat(df: DataFrame, min_support: float, item_col: str, *, max_iter: int = 100_000):
    vert_df, transact_col = vertical_transform(df, item_col)
    minFreq = ceil(min_support * len(df[transact_col].unique()))
    
    equiv_classes = [[]]
    freq_itemset = []
    for item, transacts in zip(vert_df.index, vert_df[transact_col]):
        equiv_classes[0].append(([item], transacts))
        freq_itemset.append(([item], len(transacts)))
    ...
    while equiv_classes and (max_iter := max_iter - 1):
        ...
        for i, (itemset, transacts) in enumerate(cur_class[:-1]):
            ...
            for itemset2, transacts2 in cur_class[(i + 1):]:
                ...
                new_itemset = find_common_prefix(itemset, itemset2)
...
def vertical_transform(df: DataFrame, item_col: str) -> tuple[DataFrame, str]:
...
def find_common_prefix(itemset1: list, itemset2: list) -> list:
